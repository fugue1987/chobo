...
**已经在跑旧版本?一次性升级(不丢数据):**
1. 解压新包到新目录(或覆盖旧目录)。
2. 把你原来的 `chobo-crm.env` 拷进新目录 —— 库地址/密钥不变,**你的 Postgres 和全部历史数据完全不动**。
3. **⚠ 价目文件别被覆盖**:本包里的 `price-seed.json` 是**示例/起点**(含我们的 `doubao-seed-2.1-pro` 样例)。**如果你之前改过自己的 `price-seed.json`(加过你自己的模型价/别名),把你那份拷进新目录**,别用本包的示例盖掉 —— 否则重启后本包的价目版本会成为最新版、把你的现行价目"顶掉"(旧版仍在库不丢,但不再是生效版)。没改过就用本包的即可。
4. `./start.sh` 重启一次。启动时自动跑迁移 + **价目版本增量同步**(price-seed 带的新版本自动插入,**不覆盖**任何已有版本/你的人工调价),算价与看板逻辑随包更新。

> 升级到**本版本**后就装上了「运行时价目热更新」:**以后加模型价不用再换包、也不用重启**(见下一条 + 交付指南「以后新增模型价格」)。
